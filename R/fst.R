...
fst <- read.table(args[1], header=F, col.names=c("Populations", "Population", "FixationIndex", "Type"))

fst$Collection <- args[2]

replicated.distributions <- filter(fst, Type == "replicated") %.% group_by(Collection, Populations, Population) %.% summarize(MeanFST=mean(FixationIndex), StdDevFST=sd(FixationIndex))

fst <- inner_join(fst, replicated.distributions) %.% mutate(ZScore = (FixationIndex - MeanFST) / StdDevFST)

min.value <- min(fst$ZScore)
max.value <- max(fst$ZScore)
...
dataset.mins <- fst %.% group_by(Collection) %.% summarize(min = min(FixationIndex), max = max(FixationIndex)) %.% mutate(lower = min + 0.9 * (max - min)) %.% select(Collection, lower)

real.zscores <- filter(fst, Type=="real")
...
vlines <- rbind(fst %.% group_by(Collection, Populations) %.% summarize(XIntercept = 0.5),
       	    	    fst %.% group_by(Collection, Populations) %.% summarize(XIntercept = mean(Populations) + 0.5))

p <- ggplot(filter(fst, Type=="replicated"), aes(Population + 1, FixationIndex)) + 
...
p + geom_point(data=filter(fst, Type=="real"), aes(Population + 1 - (0.04 * Populations), FixationIndex, color=ZScore), size=3.5, alpha=1.0) +
  scale_color_gradientn(colours=z.colors, values=z.values)
  
ggsave("fst.pdf", width=6, height=4)
